...
-   Один из актуальных дистрибутивов: **Ubuntu 20.04 / 22.04**, **Debian 9 / 10 / 11**, **CentOS 7 / 8**, **ArchLinux** и другие
...
- Создайте в пространстве имён своего домена **A** запись, вида `launcher.ДОМЕН.ru`, с вышим **IP** машины с лаунчсервером
::: details Путь к конфигурации Nginx:
Предпочтительно создавать отдельный файл конфигурации для каждого домена отдельно:
(Воспользуйтесь SFTP проводником)
```
...
```
Перевыдать группу и пользователя на всё содержимое домашней директории **launcher**:
```bash
