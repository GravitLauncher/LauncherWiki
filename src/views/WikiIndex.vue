<template><div class="wiki">
  ...
  <h2>Начало работы</h2>
  <b>Ставьте только те модули, что вам действительно необходимы. Большинство модулей требует дополнительной
    конфигурации</b><br>
  Модули, заканчивающиеся на <span class="codes">_module</span> - для лаунчсервера, на <span
    class="codes">_lmodule</span> - для лаунчера(требутется LauncherModuleLoader), на <span
    class="codes">_swmodule</span> для ServerWrapper'а<br>
  <ol>
    <li>
      <details>
        <summary tabindex="1">Устанавливаем лаунчсервер</summary>
        <h3>Вариант 1: Скрипт установки <div class="gtag gtag-easy">Самый простой вариант</div>
          <div class="gtag gtag-info">Рекомендуется</div>
        </h3>
        <p>Для установки лаунчера версии 5.1.3+ можно воспользоваться скриптом. Доступные на данный момент скрипты
          находятся <a href="https://mirror.gravit.pro/scripts">тут</a>.</p>
        <ol>
          <li><b>dev</b> - самая последняя версия, менее или более стабильная чем master. Доступны новейшие возможности,
            возможны проблемы</li>
          <li><b>master</b> - последний доступный релиз. Wiki всегда основана именно на этой ветке</li>
        </ol>
        <p>Для установки лаунчсервера на Linux перейдите в папку, в которой у вас будет находиться лаунчсервер и
          выполните следующие команды:</p>
        <ol>
          <li> <span class="codes">curl -o setup.sh https://mirror.gravit.pro/scripts/setup-ВЕРСИЯ.sh</span>.</li>
          <li>(ОПЦИОНАЛЬНО) Выполните <span class="codes">cat setup.sh</span> и убедитесь, что скрипт скачан без ошибок.
          </li>
          ...
          <li>Если всё прошло успешно последним сообщением будет: <span class="codes">NOT DELETE DIRECTORIES src AND
              srcRuntime</span></li>
          <li>Если что то пошло не так на фазе проверки(фаза 0) - исправьте проблему и запустите скрипт снова.<br>
            ...
            файлы и папки и заного запустить скрипт</li>
        </ol>
        <p>При установке на Windows можно воспользоваться Cygwin или другим методом установки</p>
        <h4>Требования для успешной установки лаунчсервера скриптом <div class="gtag gtag-important">Важно</div>
        </h4>
        <ul>
          <li>При выборе ОС выбирайте самые последние версии дистрибутивов: Debian 10, Ubuntu 18.04, ArchLinux и другие
          </li>
          <li>У вас должно быть достаточно оперативной памяти для сборки(хотя бы 600-700Мб свободно), сам лаунчсервер
            при этом может работать с 300Мб или даже 128Мб. Если у вас машина с малым объемом ОЗУ и скрипт сборки падает
            из за нехватки памяти можете попробывать включить swap или воспользоваться другими вариантами установки</li>
          <li>У вас должен быть установлен OpenJDK 11(и запускаться по умолчанию, т.е. вывод <span class="codes">java
              -version</span> должен выдать OpenJDK 11)</li>
          <li>Для работы ProGuard уже после установки лаунчсервера вам понадобится OpenJFX той же версии, что и ваша
            OpenJDK(11). Иначе собранный лаунчер не запустится с ошибкой AbstractMethodError на JavaFXApplication при
            включеном ProGuard</li>
          ...
          <ul>
            ...
            <li>Убедитесь что вы не меняли конфиг launch4j. Неправильное изменение некоторых параметров может привести к
              такому эффекту</li>
            <li>Если у вас 32бит Linux или у вас какие то проблемы с запуском windres попробуйте скачать <a
                href="https://repo1.maven.org/maven2/net/sf/launch4j/launch4j/3.12/launch4j-3.12-workdir-linux.jar">этот
                архив с 32битными исполняемыми файлами</a> и распаковать в папку libraries/launch4j. При этом для 64бит
              систем вам нужно будет установить 32битные библиотеки(Debian: lib32z1 | CentOS: glibc.i686 | ArchLinux:
              lib32-glibc lib32-zlib)</li>
          </ul>
        </ul>
        <h4>Как пользоваться скриптом после успешной установки лаунчсервера <div class="gtag gtag-important">Важно</div>
        </h4>
        <p>Скрипт экономит много времени и делает за вас большую часть работы. Вам <b>не нужно</b> следовать инструкции
          по установке модуля рантайма - скрипт установил его за вас</p>
        <p><i>update.sh</i> - обновляет лаунчсервер и рантайм до последней версии. При этом перед обновлением ваши
          изменения скрываются, а после обновления - восстанавливаются. Это позволяет вам обновится без конфликтов с
          сохранением ваших изменений в дизайне<br>
          Этот метод не заменит полноценной разработке рантайма с использованием Git, но тем не менее является отличным
          решением для начинающих</p>
        <p><i>client.sh ClientName</i> - после скачки клиента через downloadClient вы можете воспользоваться этим
          скриптом что бы скопировать authlib и launchwrapper в папку с клиентом. ClientName - имя папки клиента в
          updates</p>
        <p><i>start.sh</i> - Запуск лаунчсервера без использования screen. Рекомендуется для тестирования и начальной
          настройки</p>
        <p><i>startscreen.sh</i> - Запуск лаунчсервера с использованием screen. Рекомендуется для постоянной работы.
          <span class="codes">Ctrl + A + D</span> - выйти из консоли лаунчсервере не убивая процесс. <span
            class="codes">screen -x</span> - подключится к консоли лаунчсервера.<b>Следите за тем что бы не запустить
            случайно два лаунчсервера. Если вы запустите два лаунчсервера одновременно - вы можете получить странные
            баги на гране здравого смысла и долго искать решение</b></p>
        <p><b>Так как при установке лаунчсервера скриптом вы используете JDK 11, а майнкрафт(и сервера в том числе)
            1.12.2 и ниже используют Java 8 вам необходимо поставить еще и Java 8 себе на VDS, при этом изменив прямой
            вызов java в ваших скриптах старта сервера на путь к java 8-ой версии</b></p>
        <p></p>
        <h3>Вариант 2: Скачивание релиза с GitHub Actions <div class="gtag gtag-easy">Это просто</div>
        </h3>
        <p>Открываем по ссылке <a class="link-animated" href="https://github.com/GravitLauncher/Launcher/actions">GitHub
            Actions</a><br>
          ...
          распаковываем LaunchServer.jar и librzries.zip<br>
          <b>Ни в коем случае не распаковывайте всю папку modules сразу! Модули требуют дополнительной конфигурации и
        ...
        <b>Если вы не можете скачать файлы с GitHub Actions войдите/зарегистрируйтесь на GitHub либо скачайте из раздела
          релизов</b>
        <p>Следуем дальнейшем инструкциям на Wiki по настройке. Используя этот способ вам нужно устанавливать модуль
          рантайма самостоятельно</p>
        <h3>Вариант 3: Сборка из исходников <div class="gtag gtag-medium">Средний уровень</div>
        </h3>
        ...
        <ol>
          ...
          <span>git@github.com:</span> на <span>https://github.com/</span>
          <li>Устанавливаем <a class="link-animated"
              href="https://www.oracle.com/technetwork/java/javase/downloads/2133151">JDK</a></li>
          <li>Открываем в консоли папку с исходниками и выполняем <span class="codes">gradlew.bat build</span> (Windows)
            <span class="codes">sh gradlew build</span> (Linux)</li>
          <li>Готовый результат появится в <span class="codes">LaunchServer/build/libs</span>. Туда же будут скопированы
            все необходимые библиотеки</li>
          <li>Запускаем лаунчсервер командой <span class="codes">java -javaagent:LaunchServer.jar -jar
              LaunchServer.jar</span></li>
        </ol>
        <h4>Cкачивание вручную</h4>
        <ol>
          <li>Открываем репозиторий на <a class="link-animated"
              href="https://github.com/GravitLauncher/Launcher">GitHub</a>, жмем Clone or Download,
            и по желанию скачиваем <a class="link-animated" href="https://github.com/GravitLauncher/Launcher">модули</a>
          </li>
          <li>Распаковываем <b>Launcher-master.zip</b>, заходим в распакованную папку. Распаковываем тут, по желанию,
            <b>модули</b></li>
          <li>Открываем в <b>cmd</b> или <b>терминале</b> папку с исходными кодами, пользуясь командами <b>cd
              (папка)</b> и <b>ls</b> (Linux) <b>dir</b> (Windows)</li>
          <li>Устанавливаем <a class="link-animated"
              href="https://www.oracle.com/technetwork/java/javase/downloads/2133151">JDK</a></li>
          <li>Открываем в консоли папку с исходниками и выполняем <span class="codes">gradlew.bat build</span> (Windows)
            <span class="codes">sh gradlew build</span> (Linux)</li>
        ...
        <p>Следуем дальнейшем инструкциям на Wiki по настройке. Используя этот способ вам нужно устанавливать модуль
          рантайма самостоятельно</p>
        <h3>Вариант 4: Скачивание релиза с GitLab Pipelines <div class="gtag gtag-deprecated">Только для 5.1.2 и ниже
          </div>
        </h3>
        <p>Скачиваем последний релиз с <a class="link-animated"
            href="https://gitlab.com/gravitlauncherteam/Launcher/pipelines">GitLab</a><br>
          Он выглядит так: <br>
        ...
        <p>Запускаем лаунчсервер командой <span class="codes">java -javaagent:LaunchServer.jar -jar
            LaunchServer.jar</span></p>
      </details>
    </li>
    <li>
      <details>
        <summary tabindex="2">Устанавливаем рантайм</summary>
        <h2><a name="runtime_guide"></a>Сборка лаунчера на 5.1.0+ <div class="gtag gtag-important">Важно</div>
        </h2>
        <p>Если вы соберете лаунчер командой build сразу после настройки лаунчсервера - вы можете получить ошибку "GUI
          часть лаунчера не найдена"</p>
        <p>Это происходит потому, что начиная с 5.1.0 рантайм(GUI часть лаунчера) отделена от самого лаунчера и
          находится в отдельном репозитории.<br>
          <b>Ссылка на репозиторий рантайма</b>: <a
            href="https://github.com/GravitLauncher/LauncherRuntime">GitHub</a><br>
        <p><b>Для ветки dev лаунчсервера используется ветка dev рантайма. Для ветки master лаунчесрвера используется
            ветка master рантайма</b></p>
        <b>Ссылка на CI рантайма</b>: <a href="https://github.com/GravitLauncher/LauncherRuntime/actions">GitHub
          Actions</a><br>
        <b>Если вы не можете скачать файлы с GitHub Actions войдите/зарегистрируйтесь на GitHub либо скачайте из раздела
          релизов</b><br>
        <p>Инструкция по установке нового рантайма:</p>
        <ol>
          <li>Скачайте последний билд рантайма с GitHub actions или GitHub releases по ссылкам выше. Вы должны получить
            runtime.zip, содержащий дизайн и локализацию(fxml/css/png/runtime_**.properties) и JavaRuntime-xxxx.jar,
            содержащий код отображения этого дизайна и взаимодействия с пользователем.</li>
          <li>Скопируйте содержимое архива runtime.zip с css/fxml/png в папку runtime лаунчсервера</li>
          <li>Установите модуль LauncherModuleLoader_module на лаунчсервер в папку modules (<b>только для версий от
              5.1.0 до 5.1.3</b>, на версиях 5.1.4+ этот модуль встроен)</li>
    ...
    <li>
      <details>
        ...
        <p>Это конфигурация для 5.1.0. Ваш конфиг может отличаться. Все пояснения ниже приведены для ознакомления, не
          пытайтесь копировать конфиг или его часть себе</p>
        <pre v-highlightjs><code class="json">
{
  ...
  "startScript": "./start.sh" //Скрипт запуска лаунчсервера(используется только в команде restart)
    ...
    <li>Настраиваем <router-link to="/authprovider">AuthProvider</router-link> и <router-link to="/authhandler">
        AuthHandler</router-link>
    </li>
    <li>Собираем <router-link to="/clientbuild">клиент</router-link>
    </li>
    <li>Подключаем ваш майнкрафт сервер с помощью <router-link to="/serverbuild">ServerWrapper</router-link>
    </li>
    <li>Настраиваем <router-link to="/protection">защиту</router-link>
    </li>
  </ol>
  <p>Запускаем лаунчсервер командой <span class="codes">java -javaagent:LaunchServer.jar -jar LaunchServer.jar</span>
  </p>
  <hr>
  <h2>Рекомендуемые настройки безопасности для проектов <div class="gtag gtag-important">Важно</div>
  ...
  <ul>
    ...
    <li>Права на папку должны быть <b>755</b>, на private.key, LaunchServer.json и прочие конфигурации <b>640</b> или
      <b>600</b></li>
    <li>Рекомендуется использовать <span class="codes">screen</span> для удобного контроля за работой лаунчсервера</li>
    <li>Рекомендуется использовать systemd для автоматического рестарта, контроля за потреблением памяти и автозапуска
      при перезапуске VDS</li>
    <li>Прописывать какому либо из файлов лаунчера или библиотек права 777 строго запрещается</li>
    <li>Крайне аккуратно пользуйтесь параметром <b>updateExclusions</b>. Если вы пишите что то вида <span
        class="codes">"mods/railcraft"</span> то это означает "игнорировать всё что начинается с railcraft в папке mods,
      ...
      сервере в папке вашего клиента самим создать папку railcraft).<b>Единственное оправданное приминение
        updateExclusions - это если в папке находятся динамично изменяющиеся <u>неисполняемые</u> данные, так к примеру
        поступает VoxelMap</b>. Наличие в игнорируемых папках любых .jar файлов недопустимо с точки зрения безопастности
    </li>
    <li>Крайне рекомендуется использовать проксирование nginx с SSL сертификатом(можно от CloudFlare, можно от Let's
  ...
  <p>Команды ниже настолько же круты, насколько и опасны. Если вы понимаете что делаете, эти команды будут крайне
    полезны:</p>
  <pre v-highlightjs><code class="ini">
  ...
  <p>Начиная с версии 5.0.0 в лаунчере появилась консоль, которую можно открыть после авторизации, нажав справа на
    значок консоли.<br>
    В этой консоли можно выполнять команды, недоступные из GUI. По умолчанию консоль заблокирована. Для её разблокировки
    используется команда <span>unlock [key]</span><br>
    ...
    управлять лаунчсервером (при наличии прав)<br>
    Что бы удаленно управлять лаунчсервером, ваш аккаунт должен иметь право canAdmin<br>
    <!-- TODO commands list -->
  <h2>Интеграция с systemd <div class="gtag gtag-medium">Средний уровень</div>
  </h2>
  <p>Systemd - стандарт в мире дистрибутивов Linux. Ниже привожу .service файлы для лаунчсервера и сервера
    Minecraft.<br>
