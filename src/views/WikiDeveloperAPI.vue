<template><div class="wiki">
    <h2>Модули <div class="gtag gtag-hard">Программистам</div>
    </h2>
    ...
    <p>Класс модуля - это класс, наследуемый от <b>pro.gravit.launcher.modules.LauncherModule</b> и реализующий метод
        init(LauncherInitContext initContext)</p>
    <p>Реализовывать логику внутри метода init <b>запрещенно</b>. Внутри метода init разрешается обращаться только к
    ...
    <p>Всё что можно сделать из init - загружать другие модули, получать их инстансы и, самое важное, <b>подписаться на
            события</b>. Это единственный правильный способ реализовывать логику. Событий может быть великое множество,
        вы так же можете создавать свои евенты и позволять другим модулям реагировать на события вашего модуля.</p>
    <p>Встроеные общие события находятся в pro.gravit.launcher.modules.event</p>
    <p>События лаунчсервера находятся в pro.gravit.launchserver.modules.events. Именно их вы должны использовать при
        работе с лаунчсервером, так как внутри этих евентов вы получаете инстанс LaunchServer, центральный инстанс всего
        лаунчсервера, откуда можно добраться до всего</p>
    <p>События лаунчера находятся в pro.gravit.launcher.client.events. Именно их вы должны использовать при работе с
        ...
        хотите работать с GUI на 5.1.0+ то вам необходимо еще учесть события модуля Java рантайма</p>
    <p>Для подписки на событие вы должны создать метод в вашем классе, принимающий 1 аргумент соответствующий типу
        евента что вы хотите обрабатывать, после чего в init вы должны вызвать registerEvent, первый аргумент - ваш
        метод-обработчик, второй аргумент - класс интересующего вас евента</p>
    <p>Обратите внимание при загрузке модуля через loadModule евенты, которые уже прошли не вызываются, вместо этого вам
        передается initContext</p>
    <h3>Общие возможности API</h3>
    <p>Хелперы - классы помощники, упростят вам рутиные задачи. Они находятся в pro.gravit.utils.helper и <b>доступны
            извне при включенном proguard</b>(тоесть вы можете ими пользоваться в майнкрафт клиенте, модах)</p>
    <p>Сериализация абстрактных классов или интерфейсов(AuthProvider/AuthHandler и всё что имеет type в сериализованном
        виде) выполняется с помощью pro.gravit.utils.ProviderMap и pro.gravit.utils.UniversalJsonAdapter <b>доступны
            извне при включенном proguard</b></p>
    <p>Запросы к лаунчсерверу. Находятся в pro.gravit.launcher.request(ответы в pro.gravit.launcher.events) и
        <b>доступны извне при включенном proguard</b>. Блокирующий(синхронный) запрос выполняется с помощью создания
        ...
        осторожностью<br>Неблокирующий(асинхронный) заключается в использовании метода request класса
        ...
        JavaRuntime) Этот метод рекомендуется к использованию</p>
    <p>Подсистема команд. Довольно обширная и интересная система, находится в pro.gravit.utils.command. Поддерживается
        ...
        LauncherCore</p>
    <p>Хеширование директорий, Diff'ы. Находятся в pro.gravit.launcher.hasher и главная их задача - определять разницу
        между двумя директориями(недостающие/измененные и лишние файлы). На них построена система обновления</p>
    <p>Скачивание списка файлов в несколько потоков. pro.gravit.launcher.AsyncDownloader в 5.1.0+ и
        pro.gravit.launcher.downloader.ListDownloader в 5.0.X позволяют скачать список файлов по http в несколько
        потоков наиболее оптимальным способом с максимальной скоростью</p>
    <h3>Основные API лаунчсервера</h3>
    <p>Ответы на запросы. Находятся в pro.gravit.launchserver.socket.response и определяют всю логику ответа на какой
        ...
        wireshark</p>
    <p>Работа с подключенными клиентами. События. События можно отправлять и без запроса, для чего необходимо
        ...
        из модуля UnsafeCommandPack)</p>
    <p>Сборка лаунчера. pipeline. Сборка лаунчера осуществляется на основе pipeline(pro.gravit.launchserver.binary).
        ...
        поддерживается pipeline для EXE файлов.</p>
    <p>AuthProvider/AuthHandler/HWIDHandler/TextureProvider всё это можно реализовать самостоятельно, создав класс,
        отнаследовавшись от Handelr/Provider функционал которого вы хотите реализовать и зарегистрировав свой класс в
        ProviderMap соответствующего Handler/Provider.</p>
    <p>Компоненты. Компоненты - очень удобный способ конфигурировать ваш код. Помимо конфигурирования через
        ...
        ваших идей. Код компонентов и их реализация находится в pro.gravit.launchserver.components</p>
    <p>Reconfigurable - это интерфейс создания команд для вашего Handler/Provider или компонента. Работает он на
