<template><div class="wiki">
  ...
  <p>Протестированные ядра: Fabric, Sponge, Thermos, KCaldron, UltraMine, Waterfall/BungeeCoed, Spigot/Bukkit. Некоторые можно
    скачать тут: <a class="link-animated" href="https://mirror.gravit.pro/servers/">mirror.gravit.pro</a><br></p>
  ...
  <details>
    ...
    <p>Замените launchwrapper на <a href="https://mirror.gravit.pro/compat/launchwrapper-1.12-5.0.x-fixed.jar">этот</a>. Обратите внимание - нужно именно заменить - новый лаунчвраппер должен находится по тому же пути и с тем же именем что и стандартный</p>
  </details>
  <details>
    <summary tabindex="2">Сборка Fabric 1.13+</summary>
    <p>Перед запуском ядра удалите authlib из стандартного minecraft.jar(а если запустили - очистите кеш фабрика в папке .fabric)</p>
    <p>После этого докиньте файлы библиотеки apache lang3 в classpath или ServerWrapper.jar</p>
  </details>
  <details>
    <summary tabindex="2">Сборка Spigot</summary>
    <p>Докиньте файлы библиотеки apache lang3 в classpath или ServerWrapper.jar, удалив перед этим стандартную authlib из minecraft.jar</p>
  </details>
  <details>
    <summary tabindex="2">Сборка PaperSpigot</summary>
    <p>Вам необходимо пропатчить исходники при сборке патчем из архива Захара (<a href="https://cdn.discordapp.com/attachments/478133900045189122/631184267975589908/srv-patches.7z">закрепленное сообщение от Захара в канале #offtop нашего дискорд сервера</a> (файл paper.diff)<br>
    При необходимости докиньте файлы библиотеки apache lang3 в classpath или ServerWrapper.jar</p>
  </details>
  <details>
    <summary tabindex="2">Сборка Waterfall/BungeeCord</summary>
    <p>Вам необходимо пропатчить исходники при сборке патчем из архива Захара (<a href="https://cdn.discordapp.com/attachments/478133900045189122/631184267975589908/srv-patches.7z">закрепленное сообщение от Захара в канале #offtop нашего дискорд сервера</a> (3 файла, начинающиеся с bungee)</p>
  </details>
  <h2>Скрипт развертывания ServerWrapper <div class="gtag gtag-easy">Это просто</div>
  </h2>
  ...
  <p><b>Main-Class</b> - точка входа, то с чего начинается выполнение. Его можно найти открыв jar файл ядра и посмотрев
    содержимое манифеста, либо скопировать из старого скрипта запуска</p>
  <p><b>Class-Path</b> - путь, где JVM будет искать классы. Его можно найти открыв jar файл ядра и посмотрев содержимое
    манифеста, либо скопировать из старого скрипта запуска</p>
  <p><b>Profile title</b> - имя профиля клиента, его можно посмотреть открыв в лаунчсервере папку profiles, открыв файл
    своего профиля и посмотрев на поле title. title в конфигурации ServerWrapper и title в профиле должны совпадать на
    100%</p>
  <p><b>Аккаунт сервера</b> - полноценный аккаунт с логином и паролем, который будет использоватья для авторизации
    сервера</p>
...
java -cp ServerWrapper.jar:{ClassPath вашего сервера} pro.gravit.launcher.server.ServerWrapper
  ...
  <p>Для успешной авторизации сервера необходимо наличие в базе данных специального аккаунта, от имени которого будет
    совершен вход</p>
