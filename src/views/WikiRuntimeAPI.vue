  ...
  <div class="wiki">
    ...
    <ul>
      ...
      <li>
        ...
        как serverButton, уведомления и диалоги
      </li>
    ...
    <ul>
      <li>
        <b>Кастомные serverButton</b>: в папке components создайте папку
        serverButton(регистр важен) и скопируйте туда файл serverButton.fxml,
        переименовав его в PROFILE_UUID.fxml, где PROFILE_UUID - UUID вашего
        профиля, для которого вы делаете serverButton<br />
        Обратите внимание - так как папка теперь не components, а
        components/serverButton то в вашем fxml путь к папке images будет
        "@../../images" вместо "@../images"
      </li>
      <li>
        <b>Кастомные serverImage</b>: скопируйте вашу картинку в папку
        images/servers и переименуйте её в PROFILE_UUID.png, где PROFILE_UUID -
        UUID вашего профиля, для которого вы делаете serverImage
      </li>
    </ul>
    ...
    <p>
      <b>RuntimeStateMachine</b> - точка обмена данными и состоянием лаунчера
    </p>
    <p>
    ...
    <p>
      WebView - единственный компонент, который требует инициализацию в
      контексте JavaFX Thread и при попытке его добавить напрямую в fxml вы
      получите ошибку
    </p>
    <p>Для добавления WebView выполните следующие действия:</p>
    <ul>
      <li>
        Откройте build.gradle рантайма и в разделе javafx.modules допишите
        'javafx.web'
      </li>
      <li>
        В fxml подготовьте место, куда будете вставлять WebView(Pane или любой
        другой элемент-контейнер). Назначьте этому элементу подходящий fx id
      </li>
      <li>
        В методе init сцены создайте WebView и с помощью
        getChildren().add(yourWebView) добавьте его на сцену
      </li>
    </ul>
  </div>
</template>
