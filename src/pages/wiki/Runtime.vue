<template>
  <q-page padding>
    ...
    <p>
      ...
      основан на <a href='https://openjfx.io/'>JavaFX</a>. Полезные ссылки:
    </p>
    <ul>
    ...
    <ul>
      <li>Скачайте <b>архивы</b> с JRE/JDK с официального сайта Oracle или другого поставщика сборок OpenJDK</li>
  ...
  }' /><i>Формат записи: <q-badge>Java {номер версии} b{номер сборки} {mustdie (это windows)/linux/macos} x{разрядность} javafx {наличие javafx}</q-badge></i>
    ...
    <ul>
      ...
      <li>
        ...
        существовать в нескольких экземплярах одновременно, чем и отличается
        от сцен и оверлеев
      </li>
    </ul>
    <p>
    ...
    <p>
      Для редактирования текста вам понадобится папка compat, в которой находятся
      оригинальные .properties файлы.
      <b
        >Изменять напрямую файлы runtime_*.properties из папки runtime нельзя
        (не будет работать русский язык)</b
      ><br />
      После того, как вы изменили текст в compat файлах,
      воспользуйтесь утилитой native2ascii для приведения в понятный рантайму вид,
      которая идет в любом JDK.
    </p>
    ...
    <p>
      ...
      <q-badge>"@../../styles/global.css"</q-badge> вместо <q-badge>"@../styles/global.css"</q-badge><br /> 
    ...
    <ul>
      ...
      <li>
        ...
        (если параметры не отображаются, нажмите Modify Options —> Add VM options)
      </li>
    ...
    <ul>
      <li>
        ...
        пароля внедряется лаунчсервером
      </li>
      <li>
        Невозможно запустить клиент MineCraft (при этом все этапы скачивания,
        проверки и подготовки к старту будут работать)
      </li>
      <li>
